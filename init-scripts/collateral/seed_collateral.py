#!/usr/bin/env python3
"""Seed source_table.collateral with test data (run after 00_setup.sh creates
the empty table).

Usage:
    pip install -r init-scripts/requirements.txt
    python init-scripts/collateral/seed_collateral.py [--count N] [--reset]

Inserts the fixture row (reference/collateral_xml_data_sample.xml), then
generates --count more by varying only `recid` (PK / MERGE key) and `c50`
(valuation_date -- its own tag number, unrelated to account's c167),
...
TABLE = "collateral"
FIXTURE_XML = lib.REPO_ROOT / "reference" / "collateral_xml_data_sample.xml"
DATE_TAG = "c50"
DEFAULT_FIRST_RECID = 9000000170000001
